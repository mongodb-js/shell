...
var assert = require('assert'),
  ...
  utils = require('./utils'),
  tojson = types.tojson,
  tojsononeline = types.tojsononeline,
  argumentsToArray = utils.argumentsToArray,
  NumberLong = types.NumberLong,
  ...
  ObjectId,
  WriteConcern,
  DBCommandCursor = require('./query').DBCommandCursor;
...
DBCollection.prototype._dbCommand = function *(cmd, params) {
  if (typeof cmd === 'object')
    return yield this._db._dbCommand(cmd);
  var c = {};
  c[cmd] = this.getName();
  if (params)
    Object.extend(c, params);
  return yield this._db._dbCommand(c);
};
...
DBCollection.prototype.find = function *(query, fields, limit, skip, batchSize, options) {
  var cursor = yield new DBQuery(this._mongo, this._db, this, this._fullName, this._massageObject(query), fields, limit, skip, batchSize, options || this.getQueryOptions());
...
DBCollection.prototype.findOne = function *(query, fields, options) {
  var cursor = yield this.find(query, fields, -1, 0, 0, options);
  if (!cursor.hasNext())
    return null;
...
DBCollection.prototype.insert = function *(obj, options, _allow_dot) {
  ...
  var result;
  var startTime = typeof _verboseShell === 'undefined' || !_verboseShell ? 0 : new Date().getTime();
  if (this.getMongo().writeMode() !== 'legacy') {
    ...
    yield this.getMongo().insert(this._fullName, obj, flags);
    // enforce write concern, if required
    if (wc)
      result = yield this.runCommand('getLastError', wc instanceof WriteConcern ? wc.toJSON() : wc);
  }
...
DBCollection.prototype.remove = function *(t, justOne) {
  ...
  if (this.getMongo().writeMode() !== 'legacy') {
    ...
    yield this.getMongo().remove(this._fullName, this._massageObject(t), justOne ? true : false);
    // enforce write concern, if required
    if (wc)
      result = yield this.runCommand('getLastError', wc instanceof WriteConcern ? wc.toJSON() : wc);
  }
...
DBCollection.prototype.update = function *(query, obj, upsert, multi) {
  ...
  if (this.getMongo().writeMode() !== 'legacy') {
    ...
    try {
      result = yield bulk.execute(wc).toSingleResult();
    } catch (ex) {
      if (ex instanceof BulkWriteError || ex instanceof WriteCommandError) {
        result = ex.toSingleResult();
      } else {
    ...
    this.getMongo().update(this._fullName, query, obj, upsert ? true : false, multi ? true : false);
    // enforce write concern, if required
    if (wc)
      result = yield this.runCommand('getLastError', wc instanceof WriteConcern ? wc.toJSON() : wc);
  }
...
DBCollection.prototype.save = function *(obj, opts) {
  ...
  if (typeof obj._id === 'undefined') {
    ...
    return yield this.insert(obj, opts);
  } else {
    ...
    if (options.length) {
      ...
      for (var i = 0; i < options.length; i++) {
        if (typeof options[i] === 'string')
          ret.name = options[i];else if (typeof options[i] === 'boolean') {
          if (options[i]) {
            if (nb === 0)
              ret.unique = true;
            if (nb === 1)
...
DBCollection.prototype.createIndex = function *(keys, options) {
  ...
  if (this.getMongo().writeMode() === 'commands') {
    delete o.ns;
    // ns is passed to the first element in the command.
    ...
    // command response
    var result = yield this._db.getCollection('system.indexes').insert(o, 0, true);
    if (result.hasWriteError() || result.hasWriteConcernError()) {
      var error = result.hasWriteError() ? result.getWriteError() : result.getWriteConcernError();
      return {
        ok: 0,
        code: error.code,
        errmsg: error.errmsg
...
DBCollection.prototype.ensureIndex = function *(keys, options) {
  var result = yield this.createIndex(keys, options);
  if (this.getMongo().writeMode() !== 'legacy') {
    return result;
  }
  var err = yield this.getDB().getLastErrorObj();
  if (err.err) {
    return err;
...
DBCollection.prototype.reIndex = function *() {
  return yield this._db.runCommand({
    reIndex: this.getName()
  });
};
DBCollection.prototype.dropIndexes = function *() {
  if (arguments.length)
    throw 'dropIndexes doesn\'t take arguments';
  var res = yield this._db.runCommand({
...
DBCollection.prototype.drop = function *() {
  ...
  var ret = yield this._db.runCommand({
    drop: this.getName()
  });
  if (!ret.ok) {
...
DBCollection.prototype.findAndModify = function *(args) {
  ...
  var ret = yield this._db.runCommand(cmd);
  if (!ret.ok) {
...
DBCollection.prototype.renameCollection = function *(newName, dropTarget) {
  return yield this._db._adminCommand({
...
DBCollection.prototype._printExtraInfo = function *(action, startTime) {
  ...
  var res = yield this._db.getLastErrorCmd(1);
  if (res) {
...
DBCollection.prototype.validate = function *(full) {
  ...
  if (typeof full === 'object')
    // support arbitrary options here
...
DBCollection.prototype.getDiskStorageStats = function *(params) {
  var stats = yield this.diskStorageStats(params);
  if (!stats.ok) {
    print('error executing storageDetails command: ' + stats.errmsg);
    return;
  }
  print('\n    ' + 'size'.pad(9) + ' ' + '# recs'.pad(10) + ' ' + '[===occupied by BSON=== ---occupied by padding---       free           ]' + '  ' + 'bson'.pad(8) + ' ' + 'rec'.pad(8) + ' ' + 'padding'.pad(8));
  print();
  var BAR_WIDTH = 70;
  var formatSliceData = function(data) {
    ...
    return sh._dataFormat(data.onDiskBytes).pad(9) + ' ' + data.numEntries.toFixed(0).pad(10) + ' ' + bar + '  ' + (data.bsonBytes / data.onDiskBytes).toPercentStr().pad(8) + ' ' + (data.recBytes / data.onDiskBytes).toPercentStr().pad(8) + ' ' + (data.recBytes / data.bsonBytes).toFixed(4).pad(8);
  };
...
DBCollection.prototype.getPagesInRAM = function(params) {
  ...
  var formatExtentData = function(data) {
    return 'size'.pad(8) + ' ' + _barFormat([[
            data.inMem,
            '='
        ]], BAR_WIDTH) + '  ' + data.inMem.toPercentStr().pad(7);
  };
  var printExtent = function(ex, rng) {
    print('--- extent ' + rng + ' ---');
    ...
    print();
    if (ex.slices) {
      print('\tslices, percentage of pages in memory (< .1% : \' \', <25% : \'.\', ' + '<50% : \'_\', <75% : \'=\', >75% : \'#\')');
      print();
      print('\t' + 'offset'.pad(8) + '  [slices...] (each slice is ' + sh._dataFormat(ex.sliceBytes) + ')');
      line = '\t' + ('' + 0).pad(8) + '  [';
      for (var c = 0; c < ex.slices.length; c++) {
        if (c % 80 === 0 && c !== 0) {
          print(line + ']');
        ...
        if (inMem <= 0.001)
          line += ' ';else if (inMem <= 0.25)line += '.'
        ;else if (inMem <= 0.5)line += '_'
        ;else if (inMem <= 0.75)line += '='
        ;
        else
          line += '#';
      }
  ...
  if (stats.extents) {
    ...
    if (params && (params.granularity || params.numberOfSlices)) {
      ...
      print('use getPagesInRAM({granularity: _bytes_}) or ' + 'getPagesInRAM({numberOfSlices: _num_} for details');
...
DBCollection.prototype.getIndexStats = function *(params, detailed) {
  var stats = yield this.indexStats(params);
  if (!stats.ok) {
    print('error executing indexStats command: ' + tojson(stats));
    return;
  }
  print('-- index "' + stats.index + '" --');
  print('  version ' + stats.version + ' | key pattern ' + tojsononeline(stats.keyPattern) + (stats.isIdIndex ? ' [id index]' : '') + ' | storage namespace "' + stats.storageNs + '"');
  print('  ' + stats.depth + ' deep, bucket body is ' + stats.bucketBodyBytes + ' bytes');
  print();
  if (detailed) {
    print('  **  min |-- .02 quant --- 1st quartile [=== median ===] 3rd quartile --- ' + '.98 quant --| max  **  ');
    print();
  }
  ...
  var formatBoxPlot = function(st) {
    ...
    if (st.quantiles) {
      out += '\t' + fnum(st.min) + ' |-- ' + quant(st, 0.02) + ' --- ' + quant(st, 0.25) + ' [=== ' + quant(st, 0.5) + ' ===] ' + quant(st, 0.75) + ' --- ' + quant(st, 0.98) + ' --| ' + fnum(st.max) + ' ';
    }
    return out;
  };
  var formatStats = function(indent, nd) {
    var out = '';
    out += indent + 'bucket count\t' + nd.numBuckets + '\ton average ' + fnum(nd.fillRatio.mean * 100) + ' %' + ' (\xb1' + fnum(nd.fillRatio.stddev * 100) + ' %) full' + '\t' + fnum(nd.bsonRatio.mean * 100) + ' %' + ' (\xb1' + fnum(nd.bsonRatio.stddev * 100) + ' %) bson keys, ' + fnum(nd.keyNodeRatio.mean * 100) + ' %' + ' (\xb1' + fnum(nd.keyNodeRatio.stddev * 100) + ' %) key nodes\n';
    if (detailed) {
      out += indent + '\n';
  ...
  if (stats.expandedNodes) {
    ...
    for (var d = 0; d < stats.expandedNodes.length - 1; ++d) {
      ...
      if (d === 0) {
        ...
        print('  -- node # ' + params.expandNodes[d] + ' at depth ' + node.nodeInfo.depth + ' -- ');
      }
      print('    ' + (node.nodeInfo.firstKey ? tojsononeline(node.nodeInfo.firstKey) : '') + ' -> ' + (node.nodeInfo.lastKey ? tojsononeline(node.nodeInfo.lastKey) : ''));
      print('    ' + node.nodeInfo.keyCount + ' keys (' + node.nodeInfo.keyCount + ' used)' + '\tat diskloc ' + tojsononeline(node.nodeInfo.diskLoc));
      print('    ');
      ...
      if (detailed) {
        ...
        for (var k = 0; k < stats.expandedNodes[d + 1].length; ++k) {
          ...
          if (node.nodeInfo !== undefined) {
            children += node.nodeInfo.childNum + ': ' + (node.nodeInfo.fillRatio * 100).toFixed(1) + ', ' + (node.fillRatio.mean * 100).toFixed(1) + ' | ';
          } else {
            children += k + ': - | ';
          }
          if (k !== 0 && k % 5 === 0)
...
DBCollection.prototype.getShardVersion = function *() {
  return yield this._db._adminCommand({
    getShardVersion: this._fullName
  });
};
DBCollection.prototype.getIndexes = function *() {
  return yield this.getDB().getCollection('system.indexes').find({
    ns: this.getFullName()
  }).toArray();
};
...
DBCollection.prototype.getIndexKeys = function *() {
  return yield this.getIndexes().map(function(i) {
    return i.key;
  });
};
DBCollection.prototype.count = function *(x) {
  return yield this.find(x).count();
};
/**
...
DBCollection.prototype.dropIndex = function *(index) {
  ...
  var res = yield this._dbCommand('deleteIndexes', {
    index: index
  });
  return res;
};
DBCollection.prototype.copyTo = function *(newName) {
  return yield this.getDB().eval(function *(collName, newName) {
    var from = db[collName];
    var to = db[newName];
    yield to.ensureIndex({
      _id: 1
    });
    var count = 0;
    var cursor = yield from.find();
    while (yield cursor.hasNext()) {
      var o = yield cursor.next();
      count++;
      to.save(o);
    }
    return count;
...
DBCollection.prototype.convertToCapped = function *(bytes) {
  ...
  return yield this._dbCommand({
    convertToCapped: this._shortName,
    size: bytes
  });
};
DBCollection.prototype.exists = function *() {
  return yield this._db.system.namespaces.findOne({
    name: this._fullName
  });
};
DBCollection.prototype.isCapped = function *() {
  var e = yield this.exists();
  return e && e.options && e.options.capped ? true : false;
};
DBCollection.prototype._distinct = function *(keyString, query) {
  var res = yield this._dbCommand({
    distinct: this._shortName,
    key: keyString,
    query: query || {}
  });
  if (!res.ok)
...
DBCollection.prototype.distinct = function *(keyString, query) {
  var res = yield this._distinct(keyString, query);
  if (!res.ok)
...
DBCollection.prototype.aggregate = function *(pipeline, extraOpts) {
  ...
  var res = yield this.runCommand('aggregate', cmd);
  if (!res.ok && (res.code === 17020 || res.errmsg === 'unrecognized field "cursor') && !('cursor' in extraOpts)) {
...
DBCollection.prototype.group = function *(params) {
  ...
  return yield this._db.group(params);
};
DBCollection.prototype.groupcmd = function *(params) {
  params.ns = this._shortName;
  return yield this._db.groupcmd(params);
};
function MapReduceResult(db, o) {
...
MapReduceResult.prototype.find = function *() {
  if (this.results)
    return this.results;
  return yield DBCollection.prototype.find.apply(this._coll, arguments);
};
MapReduceResult.prototype.drop = function *() {
  if (this._coll) {
    return yield this._coll.drop();
  }
};
/**
...
MapReduceResult.prototype.convertToSingleObject = function *() {
  var z = {};
  var it = this.results !== null ? this.results : yield this._coll.find();
...
DBCollection.prototype.convertToSingleObject = function *(valueField) {
  var z = {};
  yield this.find().forEach(function(a) {
...
DBCollection.prototype.mapReduce = function *(map, reduce, optionsOrOutString) {
  ...
  var raw = yield this._db.runCommand(c);
  if (!raw.ok) {
    __mrerror__ = raw;
...
DBCollection.prototype.getShardDistribution = function *() {
  ...
  for (var shard in stats.shards) {
    var shardDoc = yield config.shards.findOne({
      _id: shard
    });
    print('\nShard ' + shard + ' at ' + shardDoc.host);
    var shardStats = stats.shards[shard];
    var chunks = yield config.chunks.find({
      _id: sh._collRE(this),
      shard: shard
    }).toArray();
    ...
    print(' data : ' + sh._dataFormat(shardStats.size) + ' docs : ' + shardStats.count + ' chunks : ' + chunks.length);
    print(' estimated data per chunk : ' + sh._dataFormat(estChunkData));
    print(' estimated docs per chunk : ' + estChunkCount);
  }
  print('\nTotals');
  print(' data : ' + sh._dataFormat(stats.size) + ' docs : ' + stats.count + ' chunks : ' + numChunks);
  for (var shard in stats.shards) {
    var shardStats = stats.shards[shard];
    ...
    print(' Shard ' + shard + ' contains ' + estDataPercent + '% data, ' + estDocPercent + '% docs in cluster, ' + 'avg obj size on shard : ' + sh._dataFormat(stats.shards[shard].avgObjSize));
  }
  print('\n');
};
DBCollection.prototype.getSplitKeysForChunks = function *(chunkSize) {
  ...
  var config = this.getMongo().getDB('config');
  if (!chunkSize) {
    chunkSize = yield config.settings.findOne({
      _id: 'chunksize'
    }).value;
  ...
  var shardDocs = yield config.shards.find().toArray();
  var allSplitPoints = {};
  var numSplits = 0;
  for (var i = 0; i < shardDocs.length; i++) {
    ...
    for (var j = 0; j < chunks.length; j++) {
      var chunk = chunks[j];
      var result = yield sconn.getDB('admin').runCommand({
        splitVector: this + '',
        min: chunk.min,
        max: chunk.max,
  ...
  var migration = yield config.changelog.find({
    what: /^move.*/
  }).sort({
    time: -1
  }).limit(1).toArray();
  if (migration.length === 0)
    print('\nNo migrations found in changelog.'); else {
  ...
  var splitFunction = function *() {
    ...
    print('Sleeping for 30s to allow balancers to detect change.  To be extra safe, check config.changelog' + ' for recent migrations.');
    sleep(30000);
    for (shard in allSplitPoints) {
      for (var i = 0; i < allSplitPoints[shard].length; i++) {
        ...
        printjson(yield admin.runCommand({
          split: coll + '',
          middle: splitKey
        }));
      }
...
PlanCache.prototype.help = function() {
  ...
  print('\tdb.' + shortName + '.getPlanCache().listQueryShapes() - ' + 'displays all query shapes in a collection');
  print('\tdb.' + shortName + '.getPlanCache().clear() - ' + 'drops all cached queries in a collection');
  print('\tdb.' + shortName + '.getPlanCache().clearPlansByQuery(query[, projection, sort]) - ' + 'drops query shape from plan cache');
  print('\tdb.' + shortName + '.getPlanCache().getPlansByQuery(query[, projection, sort]) - ' + 'displays the cached plans for a query shape');
  return __magicNoPrint;
};
/**
...
PlanCache.prototype._runCommandThrowOnError = function *(cmd, params) {
  var res = yield this._collection.runCommand(cmd, params);
  if (!res.ok) {
...
PlanCache.prototype.listQueryShapes = function *() {
  return yield this._runCommandThrowOnError('planCacheListQueryShapes', {}).shapes;
};
/**
 * Clears plan cache in a collection.
 */
PlanCache.prototype.clear = function *() {
  yield this._runCommandThrowOnError('planCacheClear', {});
  return;
};
/**
...
PlanCache.prototype.getPlansByQuery = function *(query, projection, sort) {
  return yield this._runCommandThrowOnError('planCacheListPlans', this._parseQueryShape(query, projection, sort)).plans;
};
/**
 * Drop query shape from the plan cache.
 */
PlanCache.prototype.clearPlansByQuery = function *(query, projection, sort) {
  yield this._runCommandThrowOnError('planCacheClear', this._parseQueryShape(query, projection, sort));
  return;
};
