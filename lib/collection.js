...
DBCollection.prototype.ensureIndex = function(keys, options) {
  ...
  var err = this.getDB().getLastErrorObj();
  if (err.err) {
    return err;
...
DBCollection.prototype.getIndexes = function() {
  return this.getDB().getCollection('system.indexes').find({
    ns: this.getFullName()
  }).toArray();
};
...
DBCollection.prototype.dropIndex = function(index) {
  ...
  var res = this._dbCommand('deleteIndexes', {
    index: index
  });
  return res;
};
...
DBCollection.prototype.exists = function() {
  return this._db.system.namespaces.findOne({
    name: this._fullName
  });
};
...
DBCollection.prototype.getSplitKeysForChunks = function(chunkSize) {
  ...
  var config = this.getMongo().getDB('config');
  if (!chunkSize) {
    chunkSize = config.settings.findOne({
      _id: 'chunksize'
    }).value;
  ...
  var migration = config.changelog.find({
    what: /^move.*/
  }).sort({
    time: -1
  }).limit(1).toArray();
...
PlanCache.prototype.getPlansByQuery = function(query, projection, sort) {
  return this._runCommandThrowOnError('planCacheListPlans', this._parseQueryShape(query, projection, sort)).plans;
};
/**
