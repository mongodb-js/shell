// autogenerated shim -->
// @todo: handle global db var
// @todo: handle evals of system.js contents.
// @todo: magic args inject on scope of MR group function?
var co = require('co');
co(function *(){
module.exports = DB;
var assert = require('assert'),
  DBCollection = require('./collection'),
  types = require('./types'),
  tojson = types.tojson,
  printjson = types.printjson,
...
DB.prototype.commandHelp = function *(name) {
  ...
  c[name] = 1;
  c.help = true;
  var res = yield this.runCommand(c);
  if (!res.ok)
    throw res.errmsg;
  return res.help;
};
DB.prototype.runCommand = function *(obj) {
  ...
  return yield this.getCollection('$cmd').findOne(obj);
};
...
DB.prototype._adminCommand = DB.prototype.adminCommand;
// alias old name
/**
...
DB.prototype.createCollection = function *(name, opt) {
  ...
  var res = yield this._dbCommand(cmd);
  return res;
};
/**
...
DB.prototype.shutdownServer = function *(opts) {
  ...
  try {
    var res = yield this.runCommand(cmd);
    if (res)
...
DB.prototype.copyDatabase = function *(fromdb, todb, fromhost, username, password) {
  ...
  if (username && password) {
    var n = this._adminCommand({
      copydbgetnonce: 1,
      fromhost: fromhost
    });
    return yield this._adminCommand({
      copydb: 1,
      fromhost: fromhost,
      fromdb: fromdb,
      todb: todb,
...
DB.prototype.printCollectionStats = function *(scale) {
  ...
  var mydb = this;
  yield this.getCollectionNames().forEach(function *(z) {
    print(z);
    printjson(yield mydb.getCollection(z).stats(scale));
    print('---');
  });
};
/**
...
DB.prototype.setProfilingLevel = function *(level, slowms) {
  ...
  return yield this._dbCommand(cmd);
};
/**
...
DB.prototype.groupcmd = function *(parmsObj) {
  var ret = yield this.runCommand({
    'group': this._groupFixParms(parmsObj)
  });
  if (!ret.ok) {
...
DB.prototype.getLastError = function *(w, wtimeout) {
  var res = yield this.getLastErrorObj(w, wtimeout);
  if (!res.ok)
...
DB.prototype.isMaster = function *() {
  return yield this.runCommand('isMaster');
};
DB.prototype.currentOp = function *(arg) {
  ...
  return yield this.$cmd.sys.inprog.findOne(q);
};
DB.prototype.currentOP = DB.prototype.currentOp;
DB.prototype.killOp = function(op) {
  if (!op)
    throw 'no opNum to kill specified';
  return this.$cmd.sys.killop.findOne({
    'op': op
  });
};
...
DB.prototype.getReplicationInfo = function *() {
  ...
  var result = {};
  var oplog;
  if (yield db.system.namespaces.findOne({
      name: 'local.oplog.rs'
    }) !== null) {
    oplog = 'oplog.rs';
  } else if (yield db.system.namespaces.findOne({
      name: 'local.oplog.$main'
    }) !== null) {
    oplog = 'oplog.$main';
  } else {
  ...
  var ol_entry = yield db.system.namespaces.findOne({
    name: 'local.' + oplog
  });
  ...
  var ol = db.getCollection(oplog);
  result.usedMB = (yield ol.stats()).size / (1024 * 1024);
  result.usedMB = Math.ceil(result.usedMB * 100) / 100;
  var firstc = yield ol.find().sort({
    $natural: 1
  }).limit(1);
  var lastc = yield ol.find().sort({
    $natural: -1
  }).limit(1);
  ...
  var first = yield firstc.next();
  var last = yield lastc.next();
  var tfirst = first.ts;
  var tlast = last.ts;
  if (tfirst && tlast) {
...
DB.prototype.printReplicationInfo = function *() {
  var result = yield this.getReplicationInfo();
  if (result.errmsg) {
...
DB.prototype.serverBuildInfo = function *() {
  return yield this._adminCommand('buildinfo');
};
DB.prototype.serverStatus = function *(options) {
  ...
  return yield this._adminCommand(cmd);
};
DB.prototype.hostInfo = function *() {
  return yield this._adminCommand('hostInfo');
};
DB.prototype.serverCmdLineOpts = function *() {
  return yield this._adminCommand('getCmdLineOpts');
};
DB.prototype.version = function *() {
  return yield this.serverBuildInfo().version;
};
DB.prototype.serverBits = function *() {
  return yield this.serverBuildInfo().bits;
};
DB.prototype.listCommands = function *() {
  var x = yield this.runCommand('listCommands');
...
DB.prototype.fsyncUnlock = function *() {
  return yield this.getSiblingDB('admin').$cmd.sys.unlock.findOne();
};
...
DB.prototype._addUserWithInsert = function *(userObj, replicatedTo, timeout) {
  ...
  userObj = Object.extend({}, userObj);
  // Prevent modifications to userObj from getting to caller
  if (userObj.pwd !== null) {
  ...
  try {
    ...
    if (tojson(e).indexOf('login') >= 0) {
      ...
      print('Creating user seems to have succeeded but threw an exception because we no ' + 'longer have auth.');
    } else {
  ...
  var le = {};
  try {
    le = yield this.getLastErrorObj(replicatedTo, timeout || 30 * 1000); // printjson( le )
  } catch ( e ) {
    var errjson = tojson(e);
  ...
  if (le.err === 'timeout') {
    throw 'timed out while waiting for user authentication to replicate - ' + 'database will not be fully secured until replication finishes';
  }
...
DB.prototype._getWriteConcern = function(replicatedTo, timeout) {
  return {
    w: replicatedTo !== null ? replicatedTo : _defaultWriteConcern.w,
...
DB.prototype._addUser = function *(userObj, replicatedTo, timeout) {
  if (typeof replicatedTo === 'object') {
    throw new Error('Cannot provide write concern object to addUser shell helper, ' + 'use createUser instead');
  }
  var commandExisted = yield this._createUser(userObj, this._getWriteConcern(replicatedTo, timeout));
  if (!commandExisted) {
    yield this._addUserWithInsert(userObj, replicatedTo, timeout);
  }
};
DB.prototype._modifyCommandToDigestPasswordIfNecessary = function(cmdObj, username) {
  ...
  if (cmdObj.hasOwnProperty('digestPassword')) {
    throw new Error('Cannot specify \'digestPassword\' through the user management shell helpers, ' + 'use \'passwordDigestor\' instead');
  }
  ...
  if (passwordDigestor === 'server') {
    ...
    throw new Error('\'passwordDigestor\' must be either \'server\' or \'client\', got: \'' + passwordDigestor + '\'');
  }
...
DB.prototype._createUser = function *(userObj, writeConcern) {
  ...
  var res = yield this.runCommand(cmdObj);
  if (res.ok) {
    print('Successfully added user: ' + getUserObjString(userObj));
    return true;
  }
  ...
  if (res.errmsg === 'timeout') {
    throw new Error('timed out while waiting for user authentication to replicate - ' + 'database will not be fully secured until replication finishes');
  }
  throw new Error('couldn\'t add user: ' + res.errmsg);
};
function _hashPassword(username, password) {
  if (typeof password !== 'string') {
    throw new Error('User passwords must be of type string. Was given password with type: ' + typeof password);
  }
...
DB.prototype._addUserDeprecatedV22Version = function *(username, pass, readOnly, replicatedTo, timeout) {
  ...
  var commandExisted = yield this._createUser(userObjForCommand, this._getWriteConcern(replicatedTo, timeout));
  if (!commandExisted) {
    var userObjForInsert = {
      user: username,
      pwd: pass,
      readOnly: readOnly || false
    };
    yield this._addUserWithInsert(userObjForInsert, replicatedTo, timeout);
  }
};
/**
...
DB.prototype.addUser = function *() {
  ...
  if (typeof arguments[0] === 'object') {
    yield this._addUser.apply(this, arguments);
  } else {
    yield this._addUserDeprecatedV22Version.apply(this, arguments);
  }
};
DB.prototype.createUser = function *(userObj, writeConcern) {
  var commandExisted = yield this._createUser(userObj, writeConcern);
  if (!commandExisted) {
    throw new Error('\'createUser\' command not found.  This is most likely because you are ' + 'talking to an old (pre v2.6) MongoDB server');
  }
};
/**
...
DB.prototype._updateUserV1 = function *(name, updateObject, writeConcern) {
  ...
  yield db.system.users.update({
    user: name,
    userSource: null
  }, {
      $set: setObj
    });
  var err = yield db.getLastError(writeConcern.w, writeConcern.wtimeout);
  if (err) {
...
DB.prototype.updateUser = function *(name, updateObject, writeConcern) {
  ...
  if (res.errmsg === 'no such cmd: updateUser') {
    yield this._updateUserV1(name, updateObject, cmdObj.writeConcern);
    return;
  }
...
DB.prototype.logout = function *() {
  return yield this.getMongo().logout(this.getName());
};
...
DB.prototype.removeUser = function *(username, writeConcern) {
  ...
  return yield db.dropUser(username, writeConcern);
};
DB.prototype.dropUser = function *(username, writeConcern) {
  ...
  if (res.errmsg === 'no such cmd: dropUsers') {
    return yield this._removeUserV1(username, cmdObj.writeConcern);
  }
...
DB.prototype._removeUserV1 = function *(username, writeConcern) {
  yield this.getCollection('system.users').remove({
    user: username
  });
  var le = yield db.getLastErrorObj(writeConcern.w, writeConcern.wtimeout);
  if (le.err) {
...
DB.prototype._authOrThrow = function *() {
  ...
  if (params.db !== undefined) {
    throw new Error('Do not override db field on db.auth(). Use getMongo().auth(), instead.');
  }
  ...
  var good = yield this.getMongo().auth(params);
  if (good) {
...
DB.prototype.auth = function *() {
  try {
    yield this._authOrThrow.apply(this, arguments);
  } catch ( ex ) {
    print(ex);
    return 0;
  }
...
DB.prototype.getUser = function *(username, args) {
  if (typeof username !== 'string') {
    throw new Error('User name for getUser shell helper must be a string');
  }
  var cmdObj = {
    usersInfo: username
  };
...
DB.prototype.getRole = function *(rolename, args) {
  if (typeof rolename !== 'string') {
    throw new Error('Role name for getRole shell helper must be a string');
  }
  var cmdObj = {
    rolesInfo: rolename
  };
