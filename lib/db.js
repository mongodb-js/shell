...
DB.prototype.currentOp = function(arg) {
  ...
  if (arg) {
    if (typeof arg === 'object')
      Object.extend(q, arg);else if (arg)q.$all = true
    ;
  }
...
DB.prototype.getReplicationInfo = function() {
  ...
  var result = {};
  var oplog;
  if (db.system.namespaces.findOne({
      name: 'local.oplog.rs'
    }) !== null) {
    oplog = 'oplog.rs';
  } else if (db.system.namespaces.findOne({
  ...
  var ol_entry = db.system.namespaces.findOne({
    name: 'local.' + oplog
  });
  ...
  var firstc = ol.find().sort({
    $natural: 1
  }).limit(1);
  var lastc = ol.find().sort({
    $natural: -1
  }).limit(1);
...
DB.prototype._addUserWithInsert = function(userObj, replicatedTo, timeout) {
  ...
  var le = {};
  try {
    le = this.getLastErrorObj(replicatedTo, timeout || 30 * 1000); // printjson( le )
  } catch (e) {
...
DB.prototype._addUserDeprecatedV22Version = function(username, pass, readOnly, replicatedTo, timeout) {
  ...
  var commandExisted = this._createUser(userObjForCommand, this._getWriteConcern(replicatedTo, timeout));
  if (!commandExisted) {
...
DB.prototype._removeUserV1 = function(username, writeConcern) {
  this.getCollection('system.users').remove({
    user: username
  });
