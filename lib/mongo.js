...
Mongo.prototype.find = function(ns, query, fields, limit, skip, batchSize, options) {
  return process.__mongo__.find(ns, query, fields, limit, skip, batchSize, options);
};
Mongo.prototype.insert = function(ns, obj) {
  return process.__mongo__.insert(ns, obj);
};
Mongo.prototype.remove = function(ns, pattern) {
  return process.__mongo__.remove(ns, pattern);
};
Mongo.prototype.update = function(ns, query, obj, upsert) {
  return process.__mongo__.update(ns, query, obj, upsert);
};

Mongo.prototype.auth = function(db, user, password) {
  return process.__mongo__.auth(db, user, password);
};
Mongo.prototype.logout = function(db) {
  return process.__mongo__.logout(db);
};
Mongo.prototype.cursorFromId = function(ns, cursorId, batchSize) {
  return process.__mongo__.cursorFromId(ns, cursorId, batchSize);
};
...
Mongo.prototype.getDBs = function() {
  var res = this.getDB('admin').runCommand({
    'listDatabases': 1
  });
  if (!res.ok)
...
Mongo.prototype.hasWriteCommands = function() {
  if (!('_hasWriteCommands' in this)) {
    var isMaster = this.getDB('admin').runCommand({
      isMaster: 1
    });
