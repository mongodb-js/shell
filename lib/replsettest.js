...
ReplSetTest.prototype.callIsMaster = function() {
  ...
  for (var i = 0; i < this.nodes.length; i++) {
    ...
    try {
      var n = this.nodes[i].getDB('admin').runCommand({
        ismaster: 1
      });
...
ReplSetTest.prototype.awaitSecondaryNodes = function(timeout) {
  ...
  assert.soon(function() {
    ...
    var ready = true;
    for (var i = 0; i < len; i++) {
      var isMaster = slaves[i].getDB("admin").runCommand({
        ismaster: 1
      });
...
ReplSetTest.prototype.status = function(timeout) {
  ...
  return master.getDB("admin").runCommand({
    replSetGetStatus: 1
  })
}
  ...
  assert.soon(function() {
    try {
      ...
      for (var i = 0; i < self.liveNodes.slaves.length; i++) {
        ...
        if (res = slave.getDB("admin").runCommand({
            replSetGetStatus: 1
          })) {
          if (res.myState == 7) {
            continue;
...
ReplSetTest.prototype.waitForIndicator = function(node, states, ind, timeout) {
  ...
  assert.soon(function() {
    ...
    for (var i = 0; i < status.members.length; i++) {
      if (printStatus) {
        print("Status for : " + status.members[i].name + ", checking " + node.host + "/" + node.name)
      }
      if (status.members[i].name == node.host || status.members[i].name == node.name) {
        for (var j = 0; j < states.length; j++) {
          if (printStatus) {
            print("Status " + " : " + status.members[i][ind] + "  target state : " + states[j])
          }
