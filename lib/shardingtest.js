...
ShardingTest = function(testName, numShards, verboseLevel, numMongos, otherParams) {
  ...
  for (var i = 0; i < numShards; i++) {
    if (otherParams.rs || otherParams["rs" + i]) {
      ...
      rsDefaults = {
        ...
        oplogSize: 40,
        pathOpts: Object.merge(pathOpts, {
          shard: i
        })
      }
      ...
      if (otherParams.shardOptions && otherParams.shardOptions.binVersion) {
        otherParams.shardOptions.binVersion = MongoRunner.versionIterator(otherParams.shardOptions.binVersion)
      }
  ...
  for (var i = 0; i < numShards; i++) {
    ...
    rs.getMaster().getDB("admin").foo.save({
      x: 1
    })
    rs.awaitReplication();
  ...
  for (var i = 0; i < (otherParams.sync ? 3 : 1); i++) {
    ...
    if (otherParams.separateConfig) {
      ...
      if (otherParams.configOptions && otherParams.configOptions.binVersion) {
        otherParams.configOptions.binVersion = MongoRunner.versionIterator(otherParams.configOptions.binVersion)
      }
  ...
  for (var i = 0; i < ( (numMongos == 0 ? -1 : numMongos) || 1 ); i++) {
    ...
    if (otherParams.mongosOptions && otherParams.mongosOptions.binVersion) {
      otherParams.mongosOptions.binVersion = MongoRunner.versionIterator(otherParams.mongosOptions.binVersion)
    }
...
ShardingTest.prototype.getServerName = function(dbname) {
  var x = this.config.databases.findOne({
    _id: "" + dbname
  });
  if (x)
    return x.primary;
...
ShardingTest.prototype.getNonPrimaries = function(dbname) {
  ...
  return this.config.shards.find({
    _id: {
      $ne: x.primary
    }
  }).map(function(z) {
...
ShardingTest.prototype.getServer = function(dbname) {
  ...
  var x = this.config.shards.findOne({
    _id: name
  });
  if (x)
    name = x.host;
...
ShardingTest.prototype.normalize = function(x) {
  var z = this.config.shards.findOne({
    host: x
  });
  if (z)
    return z._id;
...
ShardingTest.prototype.getChunksString = function(ns) {
  ...
  var s = "";
  this.config.chunks.find(q).sort({
    ns: 1,
    min: 1
  }).forEach(function(z) {
...
ShardingTest.prototype.isSharded = function(collName) {
  ...
  if (collName) {
    var x = this.config.collections.findOne({
      _id: collName
    })
    if (x) return true
    else return false
...
ShardingTest.prototype.shardGo = function(collName, key, split, move, dbName, waitForDelete) {
  ...
  var result = this.s.adminCommand({
    shardcollection: c,
    key: key
  })
  if (!result.ok) {
