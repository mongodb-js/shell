// autogenerated shim -->
// @note: exports at bottom
var DB = require('./db'),
  assert = require('assert'),
  connect = require('./mongo').connect,
  types = require('./types'),
  tojson = types.tojson,
  ...
  Mongo = require('./mongo'),
  BinData = types.BinData;

assert.soon = function() {};

// @todo: _srand and _rand?
// @todo: _replMonitorStats?

var print = console.log,
  __autocomplete__ = process.__autocomplete__,
...
var __callLastError = false;
var _verboseShell = false;

var chatty = function(s) {
  if (!__quiet)
    print(s);
};

var reconnect = function *(db) {
  yield assert.soon(function *() {
    try {
      yield db.runCommand({
        ping: 1
      });
...
var friendlyEqual = function(a, b) {
  if (a === b)
    return true;
 ...
 */
var setVerboseShell = function(value) {
  if (value === undefined)
    value = true;
  _verboseShell = value;
};
...
//      "[========================----------------------------------------                ]"
var _barFormat = function(data, width) {
  var remaining = width;
  var res = '[';
...
var compareOn = function(field) {
  return function(l, r) {
    return compare(l[field], r[field]);
  };
};
var shellPrint = function *(x) {
  if (x !== undefined)
    shellPrintHelper(x);
  if (db) {
    var e = yield db.getPrevError();
    if (e.err) {
      if (e.nPrev <= 1)
...
var setJsTestOption = function(name, value) {
  _jsTestOptions[name] = value;
};
var jsTestLog = function(msg) {
  print('\n\n----\n' + msg + '\n----\n\n');
};
var jsTest = {};
...
jsTest.authenticate = function(conn) {
  ...
  try {
    assert.soon(function() {
      ...
      conn.authenticated = true;
      print('Authenticating as internal ' + jsTestOptions().authUser + ' user with mechanism ' + DB.prototype._defaultAuthenticationMechanism + ' on connection: ' + conn);
...
jsTest.authenticateNodes = function(nodes) {
  assert.soon(function() {
    for (var i = 0; i < nodes.length; i++) {
      ...
      var res = nodes[i].getDB('admin').runCommand({
        replSetGetStatus: 1
      });
      if (res.myState === 7) {
        continue;
...
var defaultPrompt = function() {
  ...
  try {
    ...
    if (!status || !status.authRequired) {
      try {
        prompt = replSetMemberStatePrompt();
...
var replSetMemberStatePrompt = function() {
  var state = '';
  var stateInfo = db.getSiblingDB('admin').runCommand({
    replSetGetStatus: 1,
    forShell: 1
  });
  if (stateInfo.ok) {
...
var isMasterStatePrompt = function() {
  var state = '';
  var isMaster = db.runCommand({
    isMaster: 1,
    forShell: 1
  });
  if (isMaster.ok) {
    ...
    if (isMaster.setName) {
      if (isMaster.ismaster)
        role = 'PRIMARY';else if (isMaster.secondary)role = 'SECONDARY'
      ;else if (isMaster.arbiterOnly)role = 'ARBITER'
      ; else {
        role = 'OTHER';
      }
    ...
    throw new Error('Failed: ' + tojson(isMaster));
  }
  return state + '> ';
};
if (typeof _useWriteCommandsDefault === 'undefined') {
  ...
  var _useWriteCommandsDefault = function() {
    return false;
  };
}
if (typeof _writeMode === 'undefined') {
  // This is for cases when the v8 engine is used other than the mongo shell, like map reduce.
  var _writeMode = function() {
    return 'commands';
  };
}
var shellPrintHelper = function(x) {
...
var shellAutocomplete = function() {
  ...
  var builtinMethods = {};
  // uses constructor objects as keys
  ...
  builtinMethods[Boolean] = ''.split(' ');
  // nothing more than universal methods
  ...
  var isPrivate = function(name) {
    ...
    if (name[name.length - 1] === '_')
      return true;
    // some native functions have an extra name_ method
    return false;
  };
  var customComplete = function(obj) {
    try {
      if (obj.prototype.constructor.autocomplete) {
  ...
  var worker = function(prefix) {
    ...
    if (beginning.length)
      beginning += '.';
    var possibilities = [].concat(universalMethods, Object.keySet(curObj), Object.keySet(curObj.prototype), builtinMethods[curObj] || [], builtinMethods[curObj.prototype.constructor] || [], curObj === global ? extraGlobals : [], customComplete(curObj));
    var noDuplicates = {};
    ...
    for (i = 0; i < possibilities.length; i++) {
      ...
      if (typeof curObj[p] === 'undefined' && curObj !== global)
        continue;
      // extraGlobals aren't in the global object
...
shellAutocomplete.showPrivate = false;
// toggle to show (useful when working on internals)
var shellHelper = function(command, rest, shouldPrint) {
      ...
      db.system.profile.find({
        ...
        for (var z in x) {
          ...
          if (mytype === 'string' || mytype === 'number')
            l += z + ':' + val + ' ';else if (mytype === 'object')l += z + ':' + tojson(val) + ' '
  ...
  if (what === 'startupWarnings') {
    ...
    if (dbDeclared) {
      ...
      if (res.ok) {
        ...
        return '';
      } else if (res.code === 13 || res.errmsg === 'unauthorized' || res.errmsg === 'need to login') {
...
Geo.distance = function(a, b) {
  ...
  for (var key in a) {
    if (ax === null)
      ax = a[key];else if (ay === null)ay = a[key]
    ;
  }
  for (var key in b) {
    if (bx === null)
      bx = b[key];else if (by === null)by = b[key]
    ;
  }
...
Geo.sphereDistance = function(a, b) {
  ...
  for (key in a) {
    if (ax === null)
      ax = a[key] * (Math.PI / 180);else if (ay === null)ay = a[key] * (Math.PI / 180)
    ;
  }
  for (key in b) {
    if (bx === null)
      bx = b[key] * (Math.PI / 180);else if (by === null)by = b[key] * (Math.PI / 180)
    ;
  }
  var sin_x1 = Math.sin(ax),
    cos_x1 = Math.cos(ax);
  var sin_y1 = Math.sin(ay),
    cos_y1 = Math.cos(ay);
  var sin_x2 = Math.sin(bx),
    cos_x2 = Math.cos(bx);
  var sin_y2 = Math.sin(by),
    cos_y2 = Math.cos(by);
  var cross_prod = cos_y1 * cos_x1 * cos_y2 * cos_x2 + cos_y1 * sin_x1 * cos_y2 * sin_x2 + sin_y1 * sin_y2;
  if (cross_prod >= 1 || cross_prod <= -1) {
    // fun with floats
    assert(Math.abs(cross_prod) - 1 < 0.000001);
 ...
 * replica set.
 */
var _awaitRSHostViaRSMonitor = function(hostAddr, desiredState, rsName, timeout) {
  ...
  assert.soon(function() {
    ...
    return false;
  }, 'timed out waiting for replica set member: ' + hostAddr + ' to reach state: ' + tojson(desiredState), timeout);
};
rs.help = function() {
...
rs.stepDown = function(secs) {
  return db._adminCommand({
    replSetStepDown: secs === undefined ? 60 : secs
  });
};
...
rs.debug.nullLastOpWritten = function(primary, secondary) {
  ...
  var secondToLast = s.oplog.rs.find().sort({
    $natural: -1
  }).limit(1).next();
...
rs.debug.getLastOpWritten = function(server) {
  ...
  s.getMongo().setSlaveOk();
  return s.oplog.rs.find().sort({
    $natural: -1
  }).limit(1).next();
};
var help = shellHelper.help = function(x) {
  if (x === 'mr') {
...
module.exports = {
  __quiet: __quiet,
  __magicNoPrint: __magicNoPrint,
  __callLastError: __callLastError,
  chatty: chatty,
  reconnect: reconnect,
  friendlyEqual: friendlyEqual,
  printStackTrace: printStackTrace,
  setVerboseShell: setVerboseShell,
  argumentsToArray: argumentsToArray,
  _barFormat: _barFormat,
  compare: compare,
  compareOn: compareOn,
  shellPrint: shellPrint,
  jsTestName: jsTestName,
  jsTestFile: jsTestFile,
  jsTestPath: jsTestPath,
  jsTestOptions: jsTestOptions,
  setJsTestOption: setJsTestOption,
  jsTestLog: jsTestLog,
  jsTest: jsTest,
  defaultPrompt: defaultPrompt,
  isMasterStatePrompt: isMasterStatePrompt,
  replSetMemberStatePrompt: replSetMemberStatePrompt,
  _useWriteCommandsDefault: _useWriteCommandsDefault,
  _writeMode: _writeMode,
  shellPrintHelper: shellPrintHelper,
  shellAutocomplete: shellAutocomplete,
  shellHelper: shellHelper,
  Random: Random,
  Geo: Geo,
  rs: rs,
  _awaitRSHostViaRSMonitor: _awaitRSHostViaRSMonitor,
  help: help,
  print: print
};
