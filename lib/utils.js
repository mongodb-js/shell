...
var __quiet = false;
var __magicNoPrint = { __magicNoPrint: 1111 };
...
var _jsTestOptions = { enableTestCommands: true };
...
jsTest.authenticateNodes = function (nodes) {
  assert.soon(function () {
    for (var i = 0; i < nodes.length; i++) {
      ...
      var res = nodes[i].getDB('admin').runCommand({ replSetGetStatus: 1 });
      if (res.myState === 7) {
        continue;
      }
...
shellHelper.show = function (what) {
  ...
  if (what === 'profile') {
    if (db.system.profile.count() === 0) {
      ...
      print();
      db.system.profile.find({ millis: { $gt: 0 } }).sort({ $natural: -1 }).limit(5).forEach(function (x) {
  ...
  if (what === 'startupWarnings') {
    ...
    if (dbDeclared) {
      res = db.adminCommand({ getLog: 'startupWarnings' });
      if (res.ok) {
        if (res.log.length === 0) {
          return '';
        }
...
rs.stepDown = function (secs) {
  return db._adminCommand({ replSetStepDown: secs === undefined ? 60 : secs });
};
