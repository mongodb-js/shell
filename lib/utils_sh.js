    ...
    if (bytes < 1024) return Math.floor(bytes) + "B"
      if (bytes < 1024 * 1024) return Math.floor(bytes / 1024) + "KiB"
        ...
        sh.setBalancerState = function(onOrNot) {
          db.getSisterDB("config").settings.update({
            _id: "balancer"
          }, {
              $set: {
                stopped: onOrNot ? false : true
              }
            }, true);
        }

        sh.getBalancerState = function() {
          var x = db.getSisterDB("config").settings.findOne({
            _id: "balancer"
          })
          if (x == null)
            return true;
        ...
        sh.isBalancerRunning = function() {
          var x = db.getSisterDB("config").locks.findOne({
            _id: "balancer"
          });
          if (x == null) {
        ...
        sh.waitForDLock = function(lockId, onOrNot, timeout, interval) {
          ...
          if (state == undefined) {
            var currLock = db.getSisterDB("config").locks.findOne({
              _id: lockId
            })
            if (currLock != null)
              beginTS = currLock.ts
          ...
          var lockStateOk = function() {
            var lock = db.getSisterDB("config").locks.findOne({
              _id: lockId
            })
        ...
        sh.waitForPingChange = function(activePings, timeout, interval) {
          ...
          var isPingChanged = function(activePing) {
            var newPing = db.getSisterDB("config").mongos.findOne({
              _id: activePing._id
            })
        ...
        sh.disableBalancing = function(coll) {
          ...
          if (coll instanceof DBCollection)
            dbase = coll.getDB()
            dbase.getSisterDB("config").collections.update({
              _id: coll + ""
            }, {
                $set: {
                  "noBalance": true
            ...
            sh._lastMigration = function(ns) {
              ...
              if (coll)
                ...
                if (dbase)
                  ...
                  var cursor = config.changelog.find(searchDoc).sort({
                    time: -1
                  }).limit(1)
